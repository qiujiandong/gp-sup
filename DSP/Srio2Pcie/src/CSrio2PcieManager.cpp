...
int CSrio2PcieManager::startTxSeqInfoFromPC(void *dstAddr)
{
    ...
    status = startSwTxFromPC(m_hSrio2Pcie, 4, (Uint32)dstAddr, SRIO_DBRX_SWDONE);
    return status;
}
...
int CSrio2PcieManager::startTxFrameFromPC(void *dstAddr, Uint32 nSizeDW)
{
    ...
    // request for tx data while SW is busy
    do{
        status = startSwTxFromPC(m_hSrio2Pcie, nSizeDW, (Uint32)dstAddr, SRIO_DBRX_SWDONE);
    } while (status == -2);
...
int CSrio2PcieManager::startTxSwFromFPGA(void *dstAddr, Uint32 nSizeDW, Uint32 srcAddr)
{
    ...
    // request for tx data while SW is busy
    do{
        status = startSwTxFromFPGA(m_hSrio2Pcie, nSizeDW, (Uint32)dstAddr, srcAddr, SRIO_DBRX_SWDONE);
    } while (status == -2);
    return 0;
}
