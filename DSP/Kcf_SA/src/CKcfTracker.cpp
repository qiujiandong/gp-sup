 ...
 * @return int 
 */
int CKcfTracker::prepareBlocks(
    const CvMat &src,
    int nScaleCnt,
    const float *pfScales,
    CvRect *pBlocks,
    CvMat *pMaxSubw
){
    float fMaxScale;
    ...
    if(nScaleCnt < 1 || pfScales == NULL || pBlocks == NULL){
        return -1;
    }

    // NOTE: the first scale is the max scale
    fMaxScale = *pfScales;

    nWidth = round(m_roiSz.width * m_fPadding * fMaxScale);
    nHeight = round(m_roiSz.height * m_fPadding * fMaxScale);

    pBlocks[0] = cvRect(0, 0, nWidth, nHeight);

    for (int i = 1; i < nScaleCnt; ++i){
        pBlocks[i].width = round(m_roiSz.width * m_fPadding * pfScales[i]);
        pBlocks[i].x = floor((nWidth - pBlocks[i].width) * 0.5);
        pBlocks[i].height = round(m_roiSz.height * m_fPadding * pfScales[i]);
        pBlocks[i].y = floor((nHeight - pBlocks[i].height) * 0.5);
    }
    ...
    if(pMaxSubw != NULL && !bExceedBorder){
        cvGetSubRect(&src, pMaxSubw, cvRect(lu.x, lu.y, nWidth, nHeight));
        return 0;
    }

    nSubwSize = (sizeof(uint8_t) * nWidth * nHeight + 255) & (size_t)(-256);
    // pData = (uint8_t *)memalign(4096, nSubwSize);
    // assert(pData);
    ...
    if(pMaxSubw != NULL){
        cvInitMatHeader(pMaxSubw, nHeight, nWidth, CV_8UC1, pData);
        return 1;
    }
...
void CKcfTracker::init(const CvRect &roi, const CvMat &src)
{
    ...
    prepareBlocks(src, 1, &fScale, &block);
...
{
    float fPeak, fPeakLarge, fPeakSmall;
    CvPoint2D32f iLoc, iLocTmp, fCenter;
    ...
    fScales[0] = m_fScaleStep;
    fScales[1] = 1.0;
    fScales[2] = _rcpsp(m_fScaleStep);

    // detect
#if HW_FHOG_MODE

    // txBlockToFPGA(&src, 3, fScales, blocks);
    prepareBlocks(src, 3, fScales, blocks);

    // the normal scale
    getFeaturesByHwFhog(blocks[1], tmpl);
    fScaleW = m_fScaleW;
    fScaleH = m_fScaleH;
    ...
    // the large scale
    getFeaturesByHwFhog(blocks[0], tmpl);
    iLocTmp = detect(tmpl, &fPeakLarge);
    if(fPeakLarge * m_fPeakWeight > fPeak){
        iLoc = iLocTmp;
        fScaleW = m_fScaleW;
        fScaleH = m_fScaleH;
        scaleIndex = 0;
    ...
    // the small scale
    getFeaturesByHwFhog(blocks[2], tmpl);
    iLocTmp = detect(tmpl, &fPeakSmall);
    if(fPeakSmall * m_fPeakWeight > fPeak && fPeakSmall > fPeakLarge){
        iLoc = iLocTmp;
        fScaleW = m_fScaleW;
        fScaleH = m_fScaleH;
        scaleIndex = 2;
    ...
    bNeedFree = prepareBlocks(src, 3, fScales, blocks, &maxSubw);

    cvGetSubRect(&maxSubw, &subw, blocks[1]);
    getFeaturesBySw(subw, tmpl);
    // getFeatures(src, tmpl, fScales[1]);
    iLoc = detect(tmpl, &fPeak);
    fScaleW = m_fScaleW;
    fScaleH = m_fScaleH;
    scaleIndex = 1;
    ...
    // the large scale
    cvGetSubRect(&maxSubw, &subw, blocks[0]);
    getFeaturesBySw(subw, tmpl);
    iLocTmp = detect(tmpl, &fPeakLarge);
    if(fPeakLarge * m_fPeakWeight > fPeak){
        iLoc = iLocTmp;
    ...
    // the small scale
    cvGetSubRect(&maxSubw, &subw, blocks[2]);
    getFeaturesBySw(subw, tmpl);
    iLocTmp = detect(tmpl, &fPeakSmall);
    if(fPeakSmall * m_fPeakWeight > fPeak && fPeakSmall > fPeakLarge){
        iLoc = iLocTmp;
    ...
    if(bNeedFree){
        Memory_free((xdc_runtime_IHeap_Handle)hRxBufHeap, maxSubw.data.ptr, maxSubw.cols * maxSubw.rows);
        maxSubw.data.ptr = NULL;
    }

#endif

    fCenter.x = m_roiPt.x + (m_roiSz.width - 1) * 0.5;
    fCenter.y = m_roiPt.y + (m_roiSz.height - 1) * 0.5;
    m_roiSz.width = m_roiSz.width * fScales[scaleIndex];
    m_roiSz.height = m_roiSz.height * fScales[scaleIndex];

    m_roiPt.x = fCenter.x + iLoc.x * fScaleW * KCF_CELL_SIZE * fScales[scaleIndex] - (m_roiSz.width - 1) * 0.5;
    m_roiPt.y = fCenter.y + iLoc.y * fScaleH * KCF_CELL_SIZE * fScales[scaleIndex] - (m_roiSz.height - 1) * 0.5;
    ...
    fScales[0] = 1.0;
    // txBlockToFPGA(&src, 1, fScales, blocks);
    prepareBlocks(src, 1, fScales, blocks);
    getFeaturesByHwFhog(blocks[0], tmpl);
