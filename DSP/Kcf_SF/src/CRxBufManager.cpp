...
{
    if(m_pBuf[1]){
        Memory_free((xdc_runtime_IHeap_Handle)hRxBufHeap, m_pBuf[1], m_nSize);
        // free(m_pBuf[1]);
        m_pBuf[1] = NULL;
    }
    if(m_pBuf[0]){
        Memory_free((xdc_runtime_IHeap_Handle)hRxBufHeap, m_pBuf[0], m_nSize);
        // free(m_pBuf[0]);
        m_pBuf[0] = NULL;
    }
}

int CRxBufManager::init(std::size_t bufSz)
{
    Error_Block eb;
    Error_init(&eb);
    ...
    m_pBuf[0] = (uint8_t *)Memory_calloc((xdc_runtime_IHeap_Handle)hRxBufHeap, bufSz, 4096, &eb);
    // m_pBuf[0] = (uint8_t *)memalign(4096, bufSz);
    if(m_pBuf[0] == NULL){
        System_abort("Memory calloc faild\n");
    }

    m_pBuf[1] = (uint8_t *)Memory_calloc((xdc_runtime_IHeap_Handle)hRxBufHeap, bufSz, 4096, &eb);
...
int CRxBufManager::SimStartRxData(uint8_t *pSrcAddr)
{
	...
	paramSetup.aCntbCnt = CSL_EDMA3_CNT_MAKE(SIM_FRAME_FRACTION_SIZE, SIM_FRAME_FRACTIONS); // (acnt, bcnt)
	paramSetup.dstAddr = (uint32_t)getRxBuf();
	paramSetup.srcDstBidx = CSL_EDMA3_BIDX_MAKE(SIM_FRAME_FRACTION_SIZE, SIM_FRAME_FRACTION_SIZE); // (src, dst)
